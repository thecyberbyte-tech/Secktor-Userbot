...
async def gdrive_helper(_client, message):
    if len(message.text.split()) == 1:
        ...
        for drivefolders in file_list:
            if drivefolders['title'] == 'secktor Drive':
                await message.reply("Authentication successful!\nWelcome back!")
                return
        mkdir = drive.CreateFile({'title': 'secktor Drive', "mimeType": "application/vnd.google-apps.folder"})
        mkdir.Upload()
        await message.reply("Authentication successful!\nThe 'secktor Drive' folder has been created automatically!")
    else:
